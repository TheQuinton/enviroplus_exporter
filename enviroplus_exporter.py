...
AQI = Gauge('AQI', 'AQI is calculated according to the recommandations of the United States Environmental Protection Agency')
...
def get_aqi():
    """Calculate AQI from the PMS5003 sensor readings"""
    try:
        pms_data = pms5003.read()
        aqi_calc = aqi.to_aqi([
...
if __name__ == '__main__':
    ...
    parser.add_argument("-e", "--enviro", metavar='ENVIRO', type=str_to_bool, default='false', help="Device is an Enviro (not Enviro+) so don't fetch data from gas and particulate sensors as they don't exist")
